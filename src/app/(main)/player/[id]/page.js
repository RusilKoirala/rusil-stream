  ...
  const movieSources = params?.id
    ? [
        ...
        },
        {
          ...
          url: `https://multiembed.mov/directstream.php?video_id=${params.id}&tmdb=1`,
        },
        {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // Non-critical: fetch recommendations after player is interactive
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative w-full h-screen bg-black">
        <iframe
          src={movieSources[movieSourceIndex]?.url || movieSources[0]?.url}
          allowFullScreen
          className="w-full h-full border-0"
          allow="autoplay; fullscreen; picture-in-picture; encrypted-media"
          onError={() => {
            setMovieSourceIndex((current) =>
              current < movieSources.length - 1 ? current + 1 : current
            );
          }}
        />
      </div>
      ...
      <div className="border-y border-white/10 bg-[#0b0b0b]">
        <div className="max-w-6xl mx-auto px-4 md:px-8 py-4">
          <p className="text-xs uppercase tracking-wider text-gray-400 mb-3">Choose Source</p>
          <div className="flex flex-wrap gap-2">
            {movieSources.map((source, index) => (
              <button
                key={source.label}
                onClick={() => setMovieSourceIndex(index)}
                className={`px-3 py-2 rounded-lg text-sm border transition ${
                  movieSourceIndex === index
