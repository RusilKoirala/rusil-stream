...
export default function HomePageClient({
  ...
  const curatedSections = [
    { title: "Popular TV Shows", endpoint: "/api/movies?type=popular&mediaType=tv" },
    { title: "Top Rated Movies", endpoint: "/api/movies?type=top_rated&mediaType=movie" },
    { title: "Top Rated TV", endpoint: "/api/movies?type=top_rated&mediaType=tv" },
    { title: "Action Blockbusters", endpoint: "/api/movies?genre=28&mediaType=movie" },
    { title: "Comedy Picks", endpoint: "/api/movies?genre=35&mediaType=movie" },
    { title: "Sci-Fi Adventures", endpoint: "/api/movies?genre=878&mediaType=movie" },
    { title: "Animation & Family", endpoint: "/api/movies?genre=16&mediaType=movie" },
    { title: "Crime & Mystery", endpoint: "/api/movies?genre=80&mediaType=movie" },
    { title: "Documentary Spotlight", endpoint: "/api/movies?genre=99&mediaType=movie" },
  ];
    ...
    async function loadSecondary() {
      try {
        ...
        const [historyData, savedData] = await Promise.all([
          fetchJsonSafe(`/api/history?profileId=${profileId}`),
          fetchJsonSafe(`/api/saved?profileId=${profileId}`),
        ]);
        ...
        if (savedData.saved && savedData.saved.length > 0) {
          setMyList(
            savedData.saved.map((s) => ({
              id: s.movieId,
              title: s.movieTitle,
        ...
        console.error("Secondary init error:", error);
      } finally {
        setLoading(false);
      }
    }
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/movies?query=${encodeURIComponent(trimmedTerm)}&mediaType=multi`,
  ...
  function ContentCard({ item, showProgress }) {
    ...
    return (
      <Link
        href={link}
        className="group relative flex-shrink-0 w-[140px] sm:w-[160px] md:w-[200px] lg:w-[240px] transition-all duration-300 hover:scale-105 hover:z-10"
      >
        <div className="relative aspect-[2/3] rounded-lg md:rounded-xl overflow-hidden shadow-2xl">
          ...
          {showProgress && item.watchedPercentage > 0 && (
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-gray-800">
              <div className="h-full bg-blue-500 transition-all" style={{ width: `${item.watchedPercentage}%` }} />
            </div>
          )}
        </div>
  ...
  function Carousel({ title, items, showProgress = false }) {
    ...
    const scroll = (direction) => {
      ...
      const atEnd = container.scrollLeft >= maxScrollLeft - 10;
    ...
    return (
      <div className="mb-10 md:mb-14">
        <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-white mb-5 px-4 md:px-12">{title}</h2>
        <div className="relative group/carousel">
          ...
          <div ref={scrollRef} className="flex gap-2 md:gap-3 lg:gap-4 overflow-x-auto scrollbar-hide px-4 md:px-12 scroll-smooth pb-2">
            {items.map((item) => (
  ...
  function DeferredSection({ children, rootMargin = "300px" }) {
    ...
    return <div ref={sectionRef}>{visible ? children : null}</div>;
  }

  function DeferredDataSection({
    title,
    endpoint,
    canLoad,
    onLoaded,
    ...
    useEffect(() => {
      async function fetchSection() {
        if (!visible || loaded || !canLoad) return;
        try {
          const data = await fetchJsonOptional(endpoint);
      ...
      fetchSection();
    }, [visible, loaded, canLoad, endpoint, title, onLoaded]);
    ...
    return (
      <div ref={sectionRef}>
        {visible && items.length > 0 ? (
          ...
          <div className="mb-10 md:mb-14 px-4 md:px-12 min-h-[280px]">
            ...
            <div className="flex gap-3 overflow-hidden">
              {Array.from({ length: 5 }).map((_, index) => (
                <div
                  ...
                  className="w-[160px] md:w-[200px] lg:w-[240px] aspect-[2/3] rounded-xl bg-white/5 animate-pulse"
                />
              ))}
            </div>
          </div>
  ...
  if (loading && trending.length === 0 && movies.length === 0) return <Loading />;
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
              ...
              {searchTerm ? (
                <>
                  ...
                  {searchLoading ? (
                    ...
                    <div className="text-center py-20 text-gray-400 text-lg">{`No results found for "${searchTerm}"`}</div>
                  )}
                </>
              ) : (
                <div className="text-center py-20">
      ...
      <main>
        ...
        <div className="pt-8">
          {continueWatching.length > 0 && <Carousel title="Continue Watching" items={continueWatching} showProgress />}
          {myList.length > 0 && <Carousel title="My List" items={myList} />}
          ...
          {curatedSections.map((section, index) => (
            <DeferredDataSection
              ...
              canLoad={index < unlockedSections}
              onLoaded={() => setUnlockedSections((prev) => Math.max(prev, index + 2))}
            />
          ))}
          <div className="px-4 md:px-12 pb-14 pt-6 flex justify-center">
            <button
              onClick={scrollToTop}
              className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-5 py-3 rounded-xl border border-white/20 transition"
            >
