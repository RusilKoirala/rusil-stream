...
export default function TVPlayerPage(props) {
  ...
  const [tvSourceIndex, setTvSourceIndex] = useState(0);
  const router = useRouter();
  const profileIdRef = React.useRef(null);
  const controlsTimeoutRef = React.useRef(null);
  const tvSources = params?.id
    ? [
        {
          label: "Source 1 · Vidsrc",
          url: `https://vidsrc-embed.ru/embed/tv?tmdb=${params.id}&season=${selectedSeason}&episode=${selectedEpisode}${autoplay ? "&autoplay=1" : ""}`,
        },
        {
          label: "Source 2 · SuperEmbed VIP",
          url: `https://multiembed.mov/directstream.php?video_id=${params.id}&tmdb=1&s=${selectedSeason}&e=${selectedEpisode}`,
        },
        {
          label: "Source 3 · SuperEmbed Standard",
          url: `https://multiembed.mov/?video_id=${params.id}&tmdb=1&s=${selectedSeason}&e=${selectedEpisode}`,
        },
      ]
    : [];
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // Non-critical: load recommendations after player is ready
  ...
  useEffect(() => {
    setTvSourceIndex(0);
  }, [params?.id, selectedSeason, selectedEpisode, autoplay]);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative w-full aspect-video bg-black">
        <iframe
          src={tvSources[tvSourceIndex]?.url || tvSources[0]?.url}
          allowFullScreen
          className="w-full h-full border-0"
          allow="autoplay; fullscreen; picture-in-picture; encrypted-media"
          onError={() => {
            setTvSourceIndex((current) =>
              current < tvSources.length - 1 ? current + 1 : current
            );
          }}
        />
      </div>
      ...
      <div className="border-y border-white/10 bg-[#0b0b0b]">
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs uppercase tracking-wider text-gray-400 mb-2">Current Episode</p>
            <button
          ...
          <div className="md:text-right">
            <p className="text-xs uppercase tracking-wider text-gray-400 mb-2">Choose Source</p>
            <div className="flex flex-wrap gap-2 md:justify-end">
              {tvSources.map((source, index) => (
                <button
                  key={source.label}
                  onClick={() => setTvSourceIndex(index)}
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-12 py-8 md:py-12">
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              ...
              <MovieShareClient 
                url={`${typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000'}/player/tv/${params.id}`}
                title={show.name}
              />
            </div>
          </div>
